...
import org.openhab.binding.vwcarnet.internal.model.VWCarNetSmartPlugsJSON.Smartplug;
import org.openhab.binding.vwcarnet.internal.model.VWCarNetThingJSON;

/**
 * Handler for the Smart Plug Device thing type that VWCarNet provides.
 *
 * @author Jan Gustafsson - Initial contribution
 *
 */
@NonNullByDefault
public class VWCarNetSmartPlugThingHandler extends VWCarNetThingHandler {
    ...
    public VWCarNetSmartPlugThingHandler(Thing thing) {
        super(thing);
    }
    ...
    private void handleSmartPlugState(Command command) {
        ...
        if (session != null && deviceId != null) {
            VWCarNetSmartPlugsJSON smartPlug = (VWCarNetSmartPlugsJSON) session.getVWCarNetThing(deviceId);
            if (smartPlug != null) {
    ...
    public synchronized void update(@Nullable VWCarNetThingJSON thing) {
        ...
        if (getThing().getThingTypeUID().equals(THING_TYPE_SMARTPLUG)) {
            VWCarNetSmartPlugsJSON obj = (VWCarNetSmartPlugsJSON) thing;
            if (obj != null) {
    ...
    private void updateSmartPlugState(VWCarNetSmartPlugsJSON smartPlugJSON) {
