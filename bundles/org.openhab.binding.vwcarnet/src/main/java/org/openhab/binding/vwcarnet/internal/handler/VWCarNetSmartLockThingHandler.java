...
public class VWCarNetSmartLockThingHandler extends VWCarNetHandler {
    ...
    public VWCarNetSmartLockThingHandler(Thing thing) {
        super(thing);
    }
    ...
    private void handleSmartLockState(Command command) {
        ...
        if (session != null && deviceId != null) {
            VWCarNetSmartLocksJSON smartLock = (VWCarNetSmartLocksJSON) session.getVWCarNetThing(deviceId);
            if (smartLock != null) {
    ...
    public synchronized void update(@Nullable VWCarNetBaseVehicle thing) {
        ...
        if (getThing().getThingTypeUID().equals(THING_TYPE_SMARTLOCK)) {
            VWCarNetSmartLocksJSON obj = (VWCarNetSmartLocksJSON) thing;
            if (obj != null) {
    ...
    private void updateSmartLockState(VWCarNetSmartLocksJSON smartLocksJSON) {
        ...
        if (smartLockStatus != null) {
            ...
            // Fetch from "old JSON"
            VWCarNetSmartLockJSON smartLockJSON = smartLocksJSON.getSmartLockJSON();
